...
@Component({
  selector: 'app-advancepool',
  templateUrl: './advancepool.component.html',
  styleUrls: ['./advancepool.component.scss']
})
export class AdvancepoolComponent implements OnInit {

  @Input() exception : advancepool;
  ...
  ngOnInit() {
    ...
    else if(this.data.exceptions[0].exceptioninfo != null)
    {
      ...
      this.remarksList = this.removeNull(this.exceptionInfo.Remarks.split(this.arraySplitter));
...
// Finance anchor / Finance Mangager / Ops Anchor
  ...
  exinfodetails(){
    this.exceptioninfoDetails = {
      Payment_Received:this.exceptionInfo.Payment_Received,
      Pool_Balance:this.exceptionInfo.Pool_Balance,
